...
import { LoginAndRegisterService } from 'src/services/login-and-register.service';
    ...
    this.service.login(this.userLogin).subscribe(
      data=>{
        ...
        this.snackBar.open("login succesfull","X",{duration:this.durationInSeconds*1000,horizontalPosition: this.horizontalPosition,
          ...
          if(token == null){
            this.snackBar.open("token not found or expired","X",{duration:this.durationInSeconds*1000,horizontalPosition: this.horizontalPosition,
              verticalPosition: this.verticalPosition,})
          }
          else{
          this.mainservice.getUserData(this.userLogin.userName,data.token).subscribe(
            data=>{
             ...
             if(data.role==2){
                this.router.navigate(['/trainer',data.userName]);
              }
              if(data.role == 3){
                this.router.navigate(['/student',data.userName]);
              }
  },
  error=>{
    console.log(error);
